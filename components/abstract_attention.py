...
class AbstractAttention(ABC, nn.Module):
    ...
    def __init__(
        ...
        if self.cfg.load_in_4bit:
            ...
            self.W_Q = Params4bit(torch.empty(nq, 1, dtype=torch.unint8), requires_grad=False)
            self.W_O = Params4bit(torch.empty(nq, 1, dtype=torch.unint8), requires_grad=False)
        else:
        ...
        if self.cfg.positional_embedding_type == "shortformer":
            ...
            self.hook_root_k = HookPoint()
            self.hook_rot_q = HookPoint()
            if self.cfg.rotary_dim is None:
            ...
            self.alibi = None
        elif self.cfg.positional_embedding_type == "relative positional bias":
    ...
    ) -> Float[torch.Tensor, "btach pos head_index d_head"]:
        d_head = x.shape[-1]
    ...
    def calculate_qkv_matrices(
        ...
        Float[torch.Tensor, "btach kv_pos head_index d_head"],
    ]:
        attn_fn = (
            complex_attn_linear
        ...
        if self.cfg.load_in_4bit:
            if not isinstance(self.W_K, Params4bit):
                raise ValueError("W_K must be a Params4bit object is load_in_4bit is True")
            k = self.hook_k(
                bnb.matmul_4bit(
    ...
    def apply_causal_mask(
        ...
        final_mask = self.mask[None, None, -query_ctx_length:, -key_ctx_length:]
